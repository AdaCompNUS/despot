...
int SimpleTUI::run(int argc, char* argv[]) {
  ...
  if (options[E_HELP]) {
	...
	bool search_solver = options[E_SEARCH_SOLVER];
	...
	if (simulator_type == "ippc") {
		if (!options[E_SERVER]) {
			cerr << "ERROR: Specify the server name using the --server option!"
				<< endl;
			exit(0);
		}

		if (!options[E_PORT]) {
			cerr << "ERROR: Specify the port number using the --port option!"
				<< endl;
			exit(0);
		}
		...
		simulator = new IPPCEvaluator(model, belief_type, solver,
			main_clock_start, server, port, log_file, &cout);
	} else {
		if (time_limit != -1) {
			simulator = new POMDPEvaluator(model, belief_type, solver,
	...
	if (simulator_type == "ippc") {
		...
		string instance = input_file.substr(firstdot + 1,
			seconddot - firstdot - 1);

		num_runs = simulator->Handshake(instance);
		start_run = static_cast<IPPCEvaluator*>(simulator)->GetNumCompletedRuns(
			instance);
		if (num_runs == 0) {
		...
		cout << num_runs << " runs to be completed in "
			<< EvalLog::curr_inst_budget << " seconds" << endl;
	}
